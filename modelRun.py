import torch
from transformers import BertTokenizer, BertForMaskedLM, DistilBertTokenizer, DistilBertForMaskedLM, AutoTokenizer, AutoModelForMaskedLM, GPT2Tokenizer, GPT2LMHeadModel
    ...
    def inference(input_text, amount_to_generate = 5):
        ...
        with torch.no_grad():
            outputs = model.generate(
                ...
                max_new_tokens=10,              # Large enough to complete sentences
                num_return_sequences=1,
                no_repeat_ngram_size=2,
                do_sample=True,
                top_k=50,
                top_p=0.95,
                temperature=0.4,                # Lower temperature for more coherent sentences
...
input_read = open('input.txt', 'r').read()
...
for i in range(len(input_str)):
    if input_str[i] == '.' or input_str[i] == ',' or input_str[i] == '-' or input_str[i] == ';':
        input_list.append(input_str[i_start : i])
        i_start = i + 1
